...
export default function Home() {
  ...
  // 合并所有电影数据，按年份组织
  const getAllMoviesByYear = () => {
    const moviesByYear = {};
    
    const addMovies = (data, category) => {
      Object.keys(data).forEach(year => {
        if (!moviesByYear[year]) {
          moviesByYear[year] = [];
        }
        data[year].forEach(movie => {
    ...
    addMovies(moviesData, 'movies');
    addMovies(chineseMoviesData, 'chinese-movies');
    addMovies(koreanMoviesData, 'korean-movies');
    addMovies(japaneseMoviesData, 'japanese-movies');

    return moviesByYear;
  };

  const moviesByYear = getAllMoviesByYear();
  const years = Object.keys(moviesByYear).sort((a, b) => b - a);

  const getSummary = () => {
    const summary = {};
    years.forEach((year) => {
      const items = moviesByYear[year];
      const watchedItems = [];
      const unwatchedItems = [];

      items.forEach((item) => {
        const isWatched = watched[item.category]?.[item.id];
        if (isWatched) {
  ...
  const handleGenerateAIPrompt = () => {
    ...
    const categoryName = language === 'cn' ? '电影' : 'Movies';
  ...
  const handleSaveImage = async () => {
    if (mainContentRef.current) {
      try {
        ...
        link.download = `movies-${language === 'cn' ? '电影合集' : 'collection'}.png`;
        link.href = dataUrl;
        link.click();
  ...
  const totalItems = years.reduce((acc, year) => acc + moviesByYear[year].length, 0);
    ...
    <div className="min-h-screen text-gray-900 flex flex-col relative">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <h1 className="text-4xl font-bold gradient-text">
              {language === 'cn' ? '电影合集' : 'Movie Collection'}
            </h1>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="bg-white/90 backdrop-blur-lg rounded-md px-8 py-8 border border-white/60 overflow-hidden">
          {years.map((year) => {
            ...
            const yearWatched = items.filter(
              (item) => watched[item.category]?.[item.id]
            ).length;
                    ...
                    {items.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => toggleWatched(item.category, item.id)}
      ...
      <Script id="schema-org" type="application/ld+json">
        {JSON.stringify({
          ...
          "@type": "WebApplication",
          "name": language === 'cn' ? "黑猫影记 - 电影合集" : "CatFlix - Movie Collection",
          "description": language === 'cn'
            ? `黑猫影记 CatFlix 电影合集 - 浏览和记录您看过的所有电影，包括欧美电影、中国电影、韩国电影、日本电影等。共收录 ${totalItems} 部精选电影。`
            : `CatFlix Movie Collection - Browse and track all movies including Western, Chinese, Korean, and Japanese films. ${totalItems} curated films available.`,
          "url": typeof window !== 'undefined' ? window.location.origin : '',
          "applicationCategory": "EntertainmentApplication",
          "genre": "Movie Database",
          "aggregateRating": {
